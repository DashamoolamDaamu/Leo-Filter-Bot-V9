...
def _extract_quality(file_name: str) -> str | None:
    m = re.search(
        r"\b(2160p|4K|UHD|1080p|720p|480p|360p|HQ|HD|HDRip|WEB-DL|WEBRip|BluRay|BRRip|DVDRip)\b",
        file_name, re.I)
    if not m:
        return None
    raw = m.group(1).upper()
    # Normalise variants
    norm = {"WEBRIP": "WEBRip", "WEB-DL": "WEB-DL", "BRRIP": "BRRip",
            "BLURAY": "BluRay", "DVDRIP": "DVDRip", "HDRIP": "HDRip"}
...
def _apply_filters(files: list, state: dict) -> list:
    """Filter a list of file objects by the active FILTER_STATE values (lang/year/qual/s/ep)."""
    lang   = state.get("lang")
    year   = state.get("year")
    qual   = state.get("qual")
    season = state.get("s")
    ...
    for f in files:
        ...
        if lang   and _detect_language(name) != lang:
            continue
        if year   and _extract_year(name) != year:
            continue
...
def _extract_filter_meta(files: list) -> dict:
    ...
    langs, years, quals, seasons, eps = set(), set(), set(), set(), set()
    for f in files:
        name = f.file_name or ""
...
def _build_filter_buttons(key: str, req: int, offset: int,
    ...
    """
    Render 5 filter buttons in 3 rows ABOVE the file list.
        Row 1: 🌐 Language ▼   📅 Year ▼
        Row 2: 📺 Quality ▼    (full width)
        Row 3: 🎞 Season ▼     🎬 Episode ▼
    Only Year/Season/Episode shown when options exist.
    Active selections marked with ✅.
    """
    cur_lang   = state.get("lang")
    cur_year   = state.get("year")
    ...
    row1 = [
        ...
        InlineKeyboardButton(_lbl("📅 Year",     cur_year),
            callback_data=f"flt_{req}_{key}_Y"),
    ]
    rows.append(row1)

    # Row 2: Quality (full width)
    rows.append([InlineKeyboardButton(_lbl("📺 Quality", cur_qual),
        callback_data=f"flt_{req}_{key}_Q")])

    # Row 3: Season + Episode (only if options exist)
    row3 = []
    if meta.get("seasons") or cur_season:
        row3.append(InlineKeyboardButton(_lbl("🎞 Season",  cur_season),
            callback_data=f"flt_{req}_{key}_S"))
    if meta.get("eps") or cur_ep:
        row3.append(InlineKeyboardButton(_lbl("🎬 Episode", cur_ep),
...
async def _get_or_cache_all_files(key: str, search: str) -> list:
    """
    Fetch ALL results for `search` and cache in FILTER_STATE[key].
    If files are already pre-populated (e.g. by browse/actor modules), skip re-fetch.
    """
    state = FILTER_STATE.get(key, {})
    if "all_files" in state:
        return state["all_files"]   # already populated — use as-is
...
# ── Dynamic Multi-Level Filter Wizard ────────────────────────────────────────

STEP_ORDER = ("lang", "year", "qual", "season", "ep")
...
_STEP_EXTRACTOR: dict = {
    "lang":   _detect_language,
    "year":   _extract_year,
    "qual":   _extract_quality,
    "season": _parse_season,
    "ep":     _parse_episode,
}


def _get_step_values(files: list, step: str) -> list:
    fn   = _STEP_EXTRACTOR[step]
    vals = {fn(f.file_name or "") for f in files}
    vals.discard(None)
    if step == "year":
...
def _get_wizard_steps(all_files: list) -> list:
    return [s for s in STEP_ORDER if _get_step_values(all_files, s)]


def _build_wizard_step(key: str, req: int, state: dict) -> tuple | None:
    """
    Return (text, InlineKeyboardMarkup) for current wizard step.
    Returns None when all steps exhausted → show results.
    """
    steps = state.get("wizard_steps", [])
    idx   = state.get("wizard_step", 0)
    if idx >= len(steps):
        return None

    step           = steps[idx]
    label, per_row = STEP_META[step]

    all_files = state.get("all_files", [])
    # Apply already-selected filters (NOT the current step)
    tmp = {k: v for k, v in state.items() if k in STEP_ORDER and k != step}
    filtered = _apply_filters(all_files, tmp)
    options  = _get_step_values(filtered, step)

    if not options:
        # Auto-skip — nothing to pick
        state["wizard_step"] = idx + 1
        return _build_wizard_step(key, req, state)

    fn   = _STEP_EXTRACTOR[step]
    btns = []
    row  = []
    for opt in options:
        cnt     = sum(1 for f in filtered if fn(f.file_name or "") == opt)
        pfx     = "S" if step == "season" else ("E" if step == "ep" else "")
        display = f"{pfx}{opt} ({cnt})"
        row.append(InlineKeyboardButton(
            display, callback_data=f"wzv_{req}_{key}_{step}_{opt}"))
        if len(row) == per_row:
            btns.append(row); row = []
    if row: btns.append(row)

    nav = [InlineKeyboardButton("⏭ All / Skip", callback_data=f"wzskip_{req}_{key}")]
    if idx > 0:
        nav.append(InlineKeyboardButton("⬅ Back", callback_data=f"wzback_{req}_{key}"))
    btns.append(nav)
    ...
    for prev in steps[:idx]:
        val = state.get(prev)
        if val:
            ppfx = "S" if prev == "season" else ("E" if prev == "ep" else "")
            active.append(f"{STEP_META[prev][0]}: `{ppfx}{val}`")

    total  = len(filtered)
    text   = f"🔍 **Smart Filter** · Step {idx + 1}/{len(steps)}\n"
...
async def _show_wizard_results(query, key: str, req: int):
    """Show final filtered results after wizard completion."""
    state    = FILTER_STATE.get(key, {})
    settings = await get_settings(query.message.chat.id)
    pre      = "filep" if settings.get("file_secure") else "file"

    btn, total, _ = await _render_filter_page(key, req, 0, settings, pre)
        ...
        try:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔄 Reset Filters",
                        callback_data=f"wzreset_{req}_{key}"),
                ]]),
            )
    ...
    for s in STEP_ORDER:
        val = state.get(s)
        if val:
            pfx = "S" if s == "season" else ("E" if s == "ep" else "")
            active.append(f"{STEP_META[s][0]}: **{pfx}{val}**")
    ...
    try:
        await query.edit_message_text(header, reply_markup=InlineKeyboardMarkup(btn))
    except MessageNotModified:
        pass
...
async def give_filter(client, message):
    ...
    # Block if actor/director search is waiting for this user's input
    from plugins.user_states import get_state as _get_state
    if _get_state(message.from_user.id if message.from_user else 0) == "actor_search":
        return   # actor_search plugin handles this message

    # Track search for trending (async, non-blocking)
    try:
        from database.ratings_db import inc_search as _inc_s
        asyncio.create_task(_inc_s(message.text.strip().lower()[:80]))
    except Exception:
        pass
...
async def filter_pick(bot, query):
    ...
    ftype_names = {"L": "Language", "Y": "Year", "Q": "Quality", "S": "Season", "E": "Episode"}
    if value == "CLEAR":
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        from plugins.start_menu import build_start_markup, build_start_text
        mention = query.from_user.mention if query.from_user else "there"
        await query.message.edit_text(
            text=build_start_text(mention),
...
@Client.on_callback_query(filters.regex(r"^wzv_"))
async def wizard_pick(bot, query):
    """User selected a filter value."""
    parts = query.data.split("_", 4)
    if len(parts) < 5:
        return await query.answer("Invalid callback", show_alert=True)
    _, req, key, step, value = parts
    ...
    state[step] = value
    state["wizard_step"] = state.get("wizard_step", 0) + 1
    FILTER_STATE[key] = state
    await query.answer(f"✅ {value}")

    result = _build_wizard_step(key, int(req), state)
    if result:
        try:
            await query.edit_message_text(result[0], reply_markup=result[1])
        except MessageNotModified:
            pass
    else:
        await _show_wizard_results(query, key, int(req))


@Client.on_callback_query(filters.regex(r"^wzskip_"))
async def wizard_skip(bot, query):
...
@Client.on_callback_query(filters.regex(r"^wzback_"))
async def wizard_back(bot, query):
    parts = query.data.split("_", 2)
    if len(parts) < 3:
        return await query.answer("Invalid callback", show_alert=True)
    _, req, key = parts
    ...
    steps   = state.get("wizard_steps", [])
    new_idx = max(0, state.get("wizard_step", 0) - 1)
    state["wizard_step"] = new_idx
    for i in range(new_idx, len(steps)):
        state[steps[i]] = None
    FILTER_STATE[key] = state
    await query.answer()
...
@Client.on_callback_query(filters.regex(r"^wzreset_"))
async def wizard_reset(bot, query):
    parts = query.data.split("_", 2)
    if len(parts) < 3:
        return await query.answer("Invalid callback", show_alert=True)
    _, req, key = parts
    ...
    state["wizard_step"] = 0
    FILTER_STATE[key] = state
    await query.answer("Filters reset")
...
# Used by Browse, Genre, Actor, Trending, Collections — NOT by normal search.
# Shows all available filters at once on one screen with a "📂 Get Files" button.
# ══════════════════════════════════════════════════════════════════════════════

_BF_STATE: dict[int, dict] = {}   # bf_id (int) → state dict
_BF_COUNTER: int = 0
...
def _bf_apply(state: dict) -> list:
    """Apply BF_STATE filters to all_files and return filtered list."""
    files = state.get("all_files", [])
    return _apply_filters(files, state)


def build_bf_screen(bf_id: int, uid: int) -> tuple:
    """
    Build the single Browse Filter Screen — all filters visible at once.
    Returns (text, InlineKeyboardMarkup) or None if state missing.
    """
    state = _BF_STATE.get(bf_id)
    if not state:
        return None

    all_files = state.get("all_files", [])
    filtered  = _bf_apply(state)
    total     = len(filtered)
    label     = state.get("label", "Results")
    ...
    active = []
    for fld, emoji in [("lang","🌐"), ("year","📅"), ("qual","📺"), ("s","🎞"), ("ep","🎬")]:
        val = state.get(fld)
        if val:
            pfx = "S" if fld == "s" else ("E" if fld == "ep" else "")
            active.append(f"{emoji}{pfx}{val}")

    text  = f"🎬 **{label}**\n"
    if active:
        text += "Filters: " + " · ".join(active) + "\n"
    text += f"\nFound **{total}** files — select filters or get files:"

    btn = []

    # ── Language ──────────────────────────────────────────────────────────────
    langs = _get_step_values(all_files, "lang")
    if langs:
        row = []
        for lang in langs[:6]:
            cur = state.get("lang") == lang
            cnt = sum(1 for f in filtered if _detect_language(f.file_name or "") == lang)
            row.append(InlineKeyboardButton(
                f"{'✅ ' if cur else ''}🌐{lang[:6]}({cnt})",
                callback_data=f"bfv_{uid}_{bf_id}_lang_{lang}"))
            if len(row) == 3: btn.append(row); row = []
        if row: btn.append(row)

    # ── Year ──────────────────────────────────────────────────────────────────
    years = _get_step_values(all_files, "year")
    if years:
        row = []
        for yr in years[:8]:
            cur = state.get("year") == yr
            cnt = sum(1 for f in filtered if _extract_year(f.file_name or "") == yr)
            row.append(InlineKeyboardButton(
                f"{'✅ ' if cur else ''}📅{yr}({cnt})",
                callback_data=f"bfv_{uid}_{bf_id}_year_{yr}"))
            if len(row) == 4: btn.append(row); row = []
        if row: btn.append(row)

    # ── Quality ───────────────────────────────────────────────────────────────
    quals = _get_step_values(all_files, "qual")
    if quals:
        row = []
        for qual in quals[:6]:
            cur = state.get("qual") == qual
            cnt = sum(1 for f in filtered if (_extract_quality(f.file_name or "") or "").upper() == qual.upper())
            row.append(InlineKeyboardButton(
                f"{'✅ ' if cur else ''}📺{qual}({cnt})",
                callback_data=f"bfv_{uid}_{bf_id}_qual_{qual}"))
            if len(row) == 3: btn.append(row); row = []
        if row: btn.append(row)

    # ── Season ────────────────────────────────────────────────────────────────
    seasons = _get_step_values(all_files, "season")
    if seasons:
        row = []
        for s in seasons[:8]:
            cur = state.get("s") == s
            row.append(InlineKeyboardButton(
                f"{'✅ ' if cur else ''}🎞S{s}",
                callback_data=f"bfv_{uid}_{bf_id}_s_{s}"))
            if len(row) == 4: btn.append(row); row = []
        if row: btn.append(row)

    # ── Episode (only when season selected) ───────────────────────────────────
    if state.get("s"):
        eps = _get_step_values(filtered, "ep")
        if eps:
            row = []
            for ep in eps[:8]:
                cur = state.get("ep") == ep
                row.append(InlineKeyboardButton(
                    f"{'✅ ' if cur else ''}🎬E{ep}",
                    callback_data=f"bfv_{uid}_{bf_id}_ep_{ep}"))
                if len(row) == 4: btn.append(row); row = []
            if row: btn.append(row)

    # ── Action row ────────────────────────────────────────────────────────────
    nav = [InlineKeyboardButton(f"📂 Get Files ({total})", callback_data=f"bfg_{uid}_{bf_id}")]
    if any(state.get(f) for f in ["lang","year","qual","s","ep"]):
        nav.append(InlineKeyboardButton("🔄 Reset", callback_data=f"bfr_{uid}_{bf_id}"))
    btn.append(nav)
    btn.append([InlineKeyboardButton("⬅ Back", callback_data=state.get("back_cb", "bm_main"))])

    return text, InlineKeyboardMarkup(btn)


def new_bf_session(uid: int, files: list, label: str, back_cb: str = "bm_main") -> int:
    """Create a new Browse Filter session. Returns bf_id."""
    bf_id = _new_bf_id()
    _BF_STATE[bf_id] = {
        "uid": uid, "all_files": files, "label": label,
        "back_cb": back_cb,
        "lang": None, "year": None, "qual": None, "s": None, "ep": None,
    }
    return bf_id
...
@Client.on_callback_query(filters.regex(r"^bfv_(\d+)_(\d+)_(\w+)_(.+)$"))
async def bf_value_pick(bot, query):
    uid   = int(query.matches[0].group(1))
    bf_id = int(query.matches[0].group(2))
    field = query.matches[0].group(3)
    value = query.matches[0].group(4)
    ...
    # Toggle: clicking same value again deselects it
    if state.get(field) == value:
        state[field] = None
        # If deselecting season, also clear episode
        if field == "s":
            state["ep"] = None
    else:
        state[field] = value
        # If changing season, clear episode
        if field == "s":
            state["ep"] = None
...
@Client.on_callback_query(filters.regex(r"^bfr_(\d+)_(\d+)$"))
async def bf_reset(bot, query):
    uid   = int(query.matches[0].group(1))
    bf_id = int(query.matches[0].group(2))
    ...
    for f in ["lang","year","qual","s","ep"]:
        state[f] = None

    result = build_bf_screen(bf_id, uid)
    if not result: return await query.answer("Error", show_alert=True)
    text, markup = result
    await query.answer("Filters reset")
    try:
        await query.edit_message_text(text, reply_markup=markup)
...
@Client.on_callback_query(filters.regex(r"^bfg_(\d+)_(\d+)$"))
async def bf_get_files(bot, query):
    """Get Files — applies all selected browse filters and shows results."""
    uid   = int(query.matches[0].group(1))
    bf_id = int(query.matches[0].group(2))
    ...
    await query.answer(f"Loading {len(filtered)} files…")

    settings = await get_settings(query.message.chat.id)
    pre = "filep" if settings.get("file_secure") else "file"

    # Store in FILTER_STATE so existing pagination (next_) works
    bf_key = f"bf_{uid}_{bf_id}"
    BUTTONS[bf_key] = ""   # pre-populated
    FILTER_STATE[bf_key] = {
        "all_files": filtered,   # already filtered — don't filter again
        "search":    state.get("label", ""),
        "lang": None, "year": None, "qual": None, "s": None, "ep": None,
        "wizard_steps": [], "wizard_step": 0,
    }

    btn, total, _ = await _render_filter_page(bf_key, uid, 0, settings, pre)
    ...
    for fld, emoji in [("lang","🌐"),("year","📅"),("qual","📺"),("s","🎞"),("ep","🎬")]:
        ...
        if v:
            pfx = "S" if fld == "s" else ("E" if fld == "ep" else "")
            active.append(f"{emoji}{pfx}{v}")

    header = f"🎬 **{label}**"
    if active: header += "  ·  " + " · ".join(active)
    header += f"\n\nFound **{total}** files"
...
@Client.on_callback_query(filters.regex(r"^bfnxt_(\d+)_(\d+)_(\d+)$"))
async def bf_next_page(bot, query):
    uid    = int(query.matches[0].group(1))
    bf_id  = int(query.matches[0].group(2))
    offset = int(query.matches[0].group(3))

    if query.from_user.id != uid:
        return await query.answer("Not your session 🔎", show_alert=True)

    bf_key = f"bf_{uid}_{bf_id}"
    if bf_key not in FILTER_STATE:
        return await query.answer("Session expired.", show_alert=True)

    settings = await get_settings(query.message.chat.id)
    pre = "filep" if settings.get("file_secure") else "file"
    btn, total, _ = await _render_filter_page(bf_key, uid, offset, settings, pre)
