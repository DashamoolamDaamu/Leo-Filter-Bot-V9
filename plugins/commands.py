...
async def _send_rating_prompt(client, user_id: int, file_name: str):
    """Ask the user to rate the movie — non-blocking, fires 10 s after file send."""
    await asyncio.sleep(10)
    try:
        imdb = await get_poster(file_name, file=file_name) if file_name else None
        imdb_id = imdb.get("imdb_id") if imdb else None
        title   = (imdb.get("title") or file_name or "this movie")[:40] if imdb else (file_name or "")[:40]
    except Exception:
    ...
    markup = InlineKeyboardMarkup([[
        InlineKeyboardButton("👍 Good",    callback_data=f"rate_{imdb_id}_good"),
        InlineKeyboardButton("😐 Average", callback_data=f"rate_{imdb_id}_avg"),
        InlineKeyboardButton("👎 Bad",     callback_data=f"rate_{imdb_id}_bad"),
    ]])
    try:
        await client.send_message(
            chat_id=user_id,
            text=f"🎬 How was **{title}**?",
            reply_markup=markup,
        )
    except Exception:
        pass
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        from plugins.start_menu import build_start_markup
...
async def handle_rating(bot, query):
    """Handle movie rating vote: rate_{imdb_id}_{vote}"""
    parts = query.data.split("_", 2)
    if len(parts) < 3:
        return await query.answer("Invalid rating", show_alert=True)
    _, imdb_id, vote = parts
    uid = query.from_user.id
    try:
        from database.ratings_db import cast_vote, get_rating_summary
        await cast_vote(imdb_id, uid, vote)
        summary = await get_rating_summary(imdb_id)
        labels  = {"good": "👍 Good", "avg": "😐 Average", "bad": "👎 Bad"}
        await query.answer(f"Voted: {labels.get(vote, vote)}", show_alert=False)
        await query.edit_message_text(
            f"✅ **Thanks for rating!**\n\n"
            f"👍 {summary['good']}  😐 {summary['avg']}  👎 {summary['bad']}  "
            f"· Score: **{summary['score']}**"
        )
    except Exception as e:
        await query.answer("Could not save rating", show_alert=True)
